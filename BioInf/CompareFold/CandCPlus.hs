-- Cao and Chen Plus Pseudoknot Class
module BioInf.CompareFold.CandCPlus where
...
Emit: CandCPlus
|]
makeAlgebraProduct ''SigCandCPlus

bpmax :: Monad m => SigCandCPlus m Int Int Char Char
bpmax = SigCandCPlus
  { unp1   = \ c x               -> x
  , nil1   = \ ()                -> 0
  , pair1  = \ c x d y           -> if c `pairs` d then x + y + 1 else -999999
  , psk    = \ () s1 () s2 u s3 v s4 -> s1 + u + s2 + v + s3 + s4
  , nil2   = \ ()                -> 0
  , unp2   = \ c x               -> x
  , dum    = \ v                 -> v
...
pretty :: Monad m => SigCandCPlus m [String] [[String]] Char Char
pretty = SigCandCPlus
  { unp1   = \ c [x]       -> ["." ++ x]
  , nil1   = \ ()          -> [""]
  , pair1  = \ c [x] d [y] -> ["(" ++ x ++ ")" ++ y]
  , psk    = \ () [s1] () [s4] [u1,u2] [s2] [v1,v2] [s3] -> [u1 ++ s1 ++ v1 ++ s4 ++ u2 ++ s2 ++ v2 ++ s3]
  , nil2   = \ ()    -> [""]
  , unp2   = \ c [x] -> [x ++ "."]
  , dum    = \ [v1, v2] -> [v1 ++ v2]
  , pair2  = \ (Z:.a:.()) (Z:.():.b) -> ["[" ++ "]"]
...
candCPlusPairMax :: Int -> String -> (Int, [[String]])
candCPlusPairMax k inp =
...
runInsideBacktrack i (Z:.t:.u:.v:.w:.x) =
  -- error "Not Implemented"
  unId $ axiom b
  where !(Z:.b:._:._:._:._) = gCandCPlus (bpmax <|| pretty)
