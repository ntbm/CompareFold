module BioInf.CompareFold.AandU where
...
Emit: AandU
|]
makeAlgebraProduct ''SigAandU

bpmax :: Monad m => SigAandU m Int Int Char Char
bpmax = SigAandU
  { unp   = \ c x                   -> x
  , nil   = \ ()                    -> 0
...
pretty :: Monad m => SigAandU m [String] [[String]] Char Char
pretty = SigAandU
  { unp   = \ c [x] -> ["." ++ x]
  , jux   = \ c [x] d  [y] -> ["(" ++ x ++ ")" ++ y]
...
aanduPairMax :: Int -> String -> (Int, [[String]])
aanduPairMax k inp =
  (d, take k bs) where
...
runInsideBacktrack i (Z:.t:.u:.v) =
  ...
  where !(Z:.b:._:._) = gAandU (bpmax <|| pretty)
